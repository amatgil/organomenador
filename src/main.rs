...
fn main() {
    ...
    let apl387 = Rc::new(rl.load_font_from_memory(&thread, ".ttf", APL387_BYTES, UiBlock::FONT_SIZE, None).expect("Cannot fail, font is loaded in at compile time"));
    let apl387_help = rl.load_font_from_memory(&thread, ".ttf", APL387_BYTES, HELP_TEXT_FONTSIZE, None).expect("Cannot fail, font is loaded in at compile time");

    rl.set_target_fps(120);
    let help_text = {
        let mut t = String::new();
        for (k, r) in KEYMAP {
            write!(t, "{} -> Afegeix {}\n", format!("{k:?}").replace("KEY_", ""), r);
        }
    ...
    while !rl.window_should_close() {
        ...
        for block in &mut st.uiblocks { block.pos *= rescale_factor }
        ...
        if let Some(Held::Radicals(data)) = &mut st.held { for (_, f) in data { *f *= rescale_factor }}
        else if let Some(Held::Link{from,..}) = &mut st.held { *from *= rescale_factor }


        // ===== Handle clicking =====
        match (&st.held, rl.is_mouse_button_down(MouseButton::MOUSE_BUTTON_LEFT)) {
            (None, true) => {
                if let Some(index) = st.uiblocks.iter().position(|b| is_point_in_block(curr_mouse_pos, b)) {
                    st.held = Some(Held::Radicals(vec![(st.uiblocks[index].id, st.uiblocks[index].pos)]));
                }
                else if let Some((radical, from)) = link_node_at_point(&st.uiblocks,
                                                                       curr_mouse_pos,
                                                                       LINK_CIRCLE_CLICKING_THRESHOLD) {
                    st.held = Some(Held::Link { radical, from});
                } else {
                    st.held = Some(Held::RectangleCreation { from: curr_mouse_pos });
                }
            }
            (None, false)        => {}
            (Some(Held::Radicals(data)), true)  => for (id, from) in data {
            ...
            (Some(Held::Link { .. }), true) => {}
            (Some(Held::Link { radical: source_id, .. }), false) => {
                if let Some((dest_id, ..)) = link_node_at_point(&st.uiblocks, curr_mouse_pos, LINK_CIRCLE_CLICKING_THRESHOLD) {
                    let (source, dest) = get_two_blocks_unchecked_mut(&mut st.uiblocks, *source_id, dest_id );
        ...
        if rl.is_mouse_button_down(MouseButton::MOUSE_BUTTON_RIGHT) {
            match st.held {
                Some(Held::Radicals(data)) => _ = data.iter().for_each(|(id, f)| get_block_unchecked_mut(&mut st.uiblocks, *id).pos = *f),
        ...
        for (_, radical) in KEYMAP.iter().filter(|(k, _)| rl.is_key_pressed(*k)) {
            let r = 10;
            let rand_delta = Vector2 { x: rand::random_range(-r..=r) as f32, y: rand::random_range(-r..=r) as f32 };
            let b = UiBlock {
                pos: curr_mouse_pos + rand_delta,
                radical: *radical, font: apl387.clone(),
                id: rand::random(), links: vec![] 
            };
            st.push_to_undo(UiAction::AddRadical(b.clone()));
            st.uiblocks.push(b);
        }
        if rl.is_key_pressed(KeyboardKey::KEY_Z) && !rl.is_key_down(KeyboardKey::KEY_LEFT_SHIFT) { undo_last(&mut st) }
        if rl.is_key_pressed(KeyboardKey::KEY_Z) && rl.is_key_down(KeyboardKey::KEY_LEFT_SHIFT)  { redo_last(&mut st) }
        if rl.is_key_pressed(KeyboardKey::KEY_X) { delete_under_cursor(&mut st, curr_mouse_pos) }
        if rl.is_key_pressed(KeyboardKey::KEY_H) { st.is_help_up = !st.is_help_up }
        if rl.is_key_pressed(KeyboardKey::KEY_N) {
            ...
            || rl.is_mouse_button_down(MouseButton::MOUSE_BUTTON_LEFT) { st.naming_text = None }
        ...
        for block in &st.uiblocks {
            d.draw_rectangle_rounded(
                ...
                B::ROUNDNESS, B::SEGMENTS, Color::WHITE);
            d.draw_rectangle_rounded_lines_ex(
                ...
                B::ROUNDNESS, B::SEGMENTS, B::LINE_THICKNESS, Color::BLACK);

            d.draw_text_ex(&*apl387, &block.radical.to_string(), block.pos, B::FONT_SIZE as f32, B::SPACING, Color::BLACK);
        ...
        if st.naming_text.is_none() {
            d.draw_text_ex(&*apl387,
                           "H: Ajuda",
                           Vector2 { x: 5.0, y: (st.window_dims.1-HELP_TEXT_FONTSIZE) as f32 },
        ...
        if st.is_help_up {
            ...
            d.draw_rectangle(0, 0, dims.x as i32, dims.y as i32, Color::new(255u8, 255u8, 255u8, 100u8));
            d.draw_text_ex(&apl387_help, &help_text, Vector2 { x: 5.0, y: 5.0 }, HELP_TEXT_FONTSIZE as f32, B::SPACING, Color::BLACK);
        }
